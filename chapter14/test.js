...
(() => {
     ...
     */
    {
         ...
         * 결정되지 않는 프라미스 방지하기
         * 
         * 프라미스는 비동기적 코드를 단순화하고 콜백이 두 번 이상 실행되는 문제를 방지합니다. 하지만 resolve나 reject를 호
         * 출하는 걸 잊어서 프라미스가 결정되지 않는 문제까지 자동으로 해결하지는 못합니다. 에러가 일어나지 않으므로 이런 실수
         * 는 찾기 매우 어렵습니다. 복잡한 시스템에서 결정되지 않은 프라미스는 그냥 잊혀질 수 있습니다.
         * 
         * 결정되지 않은 프라미스를 방지하는 한 가지 방법은 프라미스에 타임아웃을 거는 겁니다. 충분한 시간이 지났는데도 프라미
         * 스가 결정되지 않으면 자동으로 실패하게 만들 수 있습니다. 물론 얼마나 기다려야 '충분히' 기다렸는지는 스스로 판단해야
         * 합니다. 10분 정도는 걸릴 거로 생각하는 복잡한 알고리즘에 1초 타임아웃을 걸어서는 안 됩니다.
         * 
         ...
         */
        {
            ...
            // 이 코드는 정말 무책임합니다. reject를 호출하지 않는데다가, 심지어 콘솔에 기록하지도 않습니다. 열 번 시도하면 그중
            // 다섯 번은 영문도 모른채 실패하는 셈입니다. 절대 바람직한 일이 아니죠.
            ...
            // 프라미스에 타임아웃을 걸기 위해서는 함수를 반환하는 함수가 필요한데, 절대 쉬운 문제는 아닙니다. 이 함수를 완벽히
            // 이해하는 건 상당한 고급 사용자에게나 가능한 일이니 당장 이해하지 못해도 괜찮습니다. 하지만 이 함수를 사용하는 건
            // 아주 쉽습니다. 파라미스를 반환하는 어떤 함수에든 타임아웃을 걸 수 있습니다. 로켓 과학이 엄청나게 발달해서, 가장 
            // 느린 로켓도 10초 안에는 궤도에 들어갈 수 있다고 합시다. 그러면 타임아웃은 11초면 충분합니다.

            c.go()
                .then(addTimeout(launch2, 11 * 1000))
                .then(function (msg) {
                    console.log(msg);
                })
                .catch(function (err) {
                    console.error('Houston, we have problem: ' + err.message);
                });
        }
    }
     ...
     * 제너레이터
     * 
     * 12장에서 설명했듯 제너레이터는 함수와 호출자 사이의 양방향 통신을 가능하게 합니다. 제너레이터는 원래 동기적인 성격을 가
     * 졌지만, 프라미스와 결합하면 비동기 코드를 효율적으로 관리할 수 있습니다.
     * 
     * 비동기 코드에서 가장 어려운 부분이 무엇일까요? 동기적인 코드에 비해 만들기가 어렵다는 점입니다. 어려운 문제를 해결해야 할
     * 때 우리는 대개 동기적으로 생각합니다. 1단계, 2단계, 3단계 하는 식으로 말입니다. 하지만 이렇게 하면 성능 문제가 있습니다.
     * 비동기 코드는 성능 문제를 해결하기 위해 등장했습니다. 비동기 코드의 난해함은 젖혀놓고 성능 개선만 누릴 수 있다면 정말 좋지
     * 않을까요? 제너레이터를 사용하면 일정 부분 가능해집니다.
     * 
     * 제너레이터를 사용하면 이런 자연스러운 발상과 비슷한 코드를 작성할 수 있습니다. 물론 제너레이터를 쓴다고 바로 되는 것은 아
     * 니고, 선행 작업이 약간 필요하긴 합니다.
     * 
     * 가장 먼저 할 일은 노드의 오류 우선 콜백을 프라미스로 바꾸는 겁니다. 이 기능을 nfcall(Node functino call) 함수로 만들 겠
     * 습니다.
     */
    {
     ...
     * 요약
     * 
     * 비동기적 프로그래밍의 복잡함, 그리고 그 복잡함을 덜기 위해 개발된 다양한 메커니즘을 완전히 이해하는 건 좀 어렵긴 하지만,
     * 최신 자바스크립트 개발 동향을 따라잡기 위해서는 필요한 일입니다. 이 장에서는 다음과 같은 내용을 배웠습니다.
     * 
     ...
     *  * 프라미스와 제너레이터 실행기를 결합하면 비동기적 실행의 장점을 그대로 유지하면서도 동기적인 사고방식으로 문제를 해결
     *     할 수 있습니다.
     *  * 제너레이터를 써서 동기적인 사고방식으로 문제를 해결할 때는 프로그램의 어느 부분을 동시에 실행할 수 있는지 잘 살펴야
     *     합니다. 동시에 실행할 수 있는 부분은 Promise.all을 써서 실행하십시오.
     *  * 제너레이터 실행기를 직접 만드는 고생을 하지 말고 co나 Koa를 쓰십시오.
     *  * 노드 스타일 콜백을 프라미스로 바꾸는 고생도 필요없습니다. Q를 쓰십시오.
