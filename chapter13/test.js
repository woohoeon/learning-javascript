...
(() => {
     ...
     * 재귀
     * 
     * 재귀(recursion) 역시 널리 쓰이며 함수를 활용하는 중요한 패턴입니다. 재귀란 자기 자신을 호출하는 함수입니다. 같은
     * 일을 반복하면서 그 대상이 점차 줄어드는 상황에서 재귀를 유용하게 활용할 수 있습니다.
     * 
     * 먼저 건초 더미(haystack)에서 바늘(needle)을 찾는, 좀 인위적인 예제를 하나 봅시다. 실제로 건초 더미에서 바늘을
     * 찾아야 하는 상황이라면 아마 이런 식으로 찾아볼 겁니다.
     * 1. 건초 더미에서 바늘이 보이면 3단계로 이동한다.
     * 2. 건초 더미에서 건초를 하나 덜어낸다. 1단계로 이동한다.
     * 3. 찾았다!
     * 
     * 결국 바늘을 찾을 때까지 건초 더미에서 건초를 하나씩 제외하는 소거법이며, 이것이 재귀입니다. 이 행동을 코드로 바꿔 봅
     * 시다.
     */
    {
        ...
        // 이 재귀 함수에서 눈여겨볼 점은 모든 가능성을 전부 고려한다는 겁니다. 가능한 경우의 수는 haystack이 비어 있거나
        // (찾을 대상이 없습니다), 배열의 첫 번째 요소가 바늘이거나(찾았습니다), 배열의 첫 번째 요소가 바늘이 아닌 경우입니다.
        // 마지막은 배열의 어딘가에 바늘이 들어있을 테니 Array.prototype.shift로 배열의 첫 번째 요소를 제거하고 함수를 
        // 반복 합니다.

        // 이번에는 좀 더 유용하고, 여러번 검증된 예제를 살펴봅시다. 숫자의 계승(factorial)을 찾는 예제 입니다. 숫자의 계승은
        // 1부터 그 숫자까지 전부 곱한 값이며 숫자 뒤에 느낌표를 붙여서 표시합니다. 즉 4!는 4 x 3 x 2 x 1 = 24입니다.
        ...
        // 이 함수의 종료 조건은 n === 1이고, 재귀 호출할 때마다 숫자 n은 1씩 줄어들다가 결국 1이 됩니다. 이 함수에 0이나
        // 음수를 넘겨서 호출하면 물론 에러가 생기지만, 그런 상황을 막을 조건문을 넣는 건 쉽습니다.
    }

    /**
     * 요약
     * 
     * ML이나 하스켈, 클로저(Clojure), F# 같은 다른 함수형 언어를 사용해봤다면 이 장은 아주 쉬웠을 겁니다. 같은 일을 매우 다양
     * 한 방법으로 할 수 있다보니, 어떤 방법이 '더 좋은' 방법인지 궁금할 수도 있습니다.
     * 
     * 유감이지만 그 질문에는 정답이 없습니다. 대개는 상황에 따라 답이 다릅니다. 어떤 종류의 문제는 특정한 테크닉을 요구하기도 합
     * 니다. 또 프로그래머에 따라 다르기도 합니다. 이 장을 읽으면서 각자 마음에 드는 방법이 있었을 겁니다. 이 장에서 소개한 개념은
     * 대단히 강력한 것들이고, 이들을 유용하게 쓸 수 있을지 판단하려면 시간을 두고 이해할 수밖에 없습니다. 
     */
